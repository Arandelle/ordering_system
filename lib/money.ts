...
export function fromCentavosToPeso(centavoAmount: number): number {
  return centavoAmount / CENTAVOS_PER_PESO;
}

/** Add two peso amounts using centavo arithmetic, return in pesos. */
export function addMoney(a: number, b: number): number {
  return fromCentavosToPeso(toCentavos(a) + toCentavos(b));
}
...
export function multiplyMoney(pesoAmount: number, factor: number): number {
  return fromCentavosToPeso(Math.round(toCentavos(pesoAmount) * factor));
}
...
export function roundMoney(value: number): number {
  return fromCentavosToPeso(toCentavos(value));
}
...
export function clampMoneyMin(value: number, min: number = 0): number {
  return fromCentavosToPeso(Math.max(toCentavos(value), toCentavos(min)));
}

/** Return the smaller of two peso amounts, using centavo comparison. */
export function minMoney(a: number, b: number): number {
  return fromCentavosToPeso(Math.min(toCentavos(a), toCentavos(b)));
}
