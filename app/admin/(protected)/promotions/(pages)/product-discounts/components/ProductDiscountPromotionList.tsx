...
export function ProductDiscountPromotionList({
  promotions,
}: ProductDiscountPromotionListProps) {
  const router = useRouter();
  const deletePromotion = useDeleteProductDiscountPromotion();
  const [expandedPromotionId, setExpandedPromotionId] = useState<string | null>(
    null,
  );

  const handleDelete = (promotion: ProductDiscountPromotion) => {
      ...
      {promotions.length === 0 ? (
        <div className="rounded-lg border border-dashed border-stone-200 px-4 py-10 text-center">
          <p className="text-sm font-semibold text-stone-700">
            No product discount promotions yet.
          </p>
          <p className="mt-1 text-sm text-stone-500">
            Create one to discount selected products for a scheduled period.
          </p>
        </div>
      ) : (
                  ...
                  <Fragment key={promotion._id}>
                    <tr className="border-b border-stone-100 last:border-0">
                      ...
                      <td className="px-3 py-4 text-stone-600">
                        {promotion.products.length} item
