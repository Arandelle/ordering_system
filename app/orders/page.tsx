...
import { Clock, Package, Truck, Star, ShoppingCart, X, CheckCircle, Eye, Ban, CreditCard } from "lucide-react";

const TABS = [
  { key: "all", label: "All" },
  { key: "pending", label: "To Pay" },
  { key: "preparing", label: "To Dispatch" },
  { key: "completed", label: "To Receive" },
...
export default function OrdersPage() {
  ...
  const filteredOrders = useMemo(() => {
    if (activeTab === "all") return placedOrders;
    if (activeTab === "review") return placedOrders.filter(o => o.status === "completed");
    if (activeTab === "pending") return placedOrders.filter(o => o.status === "pending");
    if (activeTab === "preparing") return placedOrders.filter(o => o.status === "paid" || o.status === "preparing");
    if (activeTab === "completed") return placedOrders.filter(o => o.status === "completed");
    return placedOrders;
                  ...
                  <div className="mt-6 pt-4 border-t border-gray-100 space-y-2">
                    ...
                    {order.totals.deliveryFee > 0 && (
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium">₱{order.totals.deliveryFee.toFixed(2)}</span>
                      </div>
                    )}
                    <div className="flex justify-between text-lg font-bold pt-2 border-t border-gray-200">
                      <span className="text-gray-900">Total</span>
                      <span className="text-[#e13e00]">₱{order.totals.total.toFixed(2)}</span>
                    </div>
                  </div>
                </div>
...
function StatusBadge({ status }: { status: 'pending' | 'paid' | 'preparing' | 'completed' | 'cancelled' }) {
  const map: Record<string, { label: string; color: string; icon: React.JSX.Element }> = {
    pending: { 
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-xs font-bold ${item.color}`}>
